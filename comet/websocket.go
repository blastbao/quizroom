...
func (server *Server) serveWebsocket(conn *websocket.Conn, tr *itime.Timer, rAddr net.Addr) {
	var (
		err error
		key string
	...
	trd = tr.Add(server.Options.HandshakeTimeout, func() {
		log.Debug("timeout rAddr: %v", rAddr)
		conn.Close()
	})
	...
	if err != nil {
		conn.Close()
		tr.Del(trd)
		log.Error("handshake failed error(%v), rAddr: %v , proto : %s", err, rAddr, string(p.Body))
		return
	}
	trd.Key = key
	tr.Set(trd, hb)
	...
	for {
		...
		if p.Operation == define.OP_HEARTBEAT {
			...
			// process message
			if err = server.operator.Operate(p, key, ch.RoomId); err != nil {
				break
			}
		}
		ch.CliProto.SetAdv()
...
func (server *Server) authWebsocket(conn *websocket.Conn, p *proto.Proto) (key string, rid int32, heartbeat time.Duration, err error) {
	...
	if p.Operation != define.OP_AUTH {
		authBody := proto.ReturnBody{Code: define.STATUS_AUTH_UNAUTHORIZED, Msg: "unauthorized!"}
	...
	authBody := proto.ReturnBody{Code: define.STATUS_AUTH_SUCCESS, Msg: "auth success!"}
    vByte, err = json.Marshal(authBody)
    if err != nil {
    	return
	}
